...
;; 모든 좌표를 감싸는 bounding box 를 구합니다. (min x, min y)
;; bounding box 를 포함하여 안에 들어가는 모든 정수 좌표 grid를 구합니다.
;; grid 의 각 점을 순회하며, 가장 가까운 타겟 좌표의 ID 를 할당합니다. 만약 가장 가까운 좌표가 2개 이상이라면 . 을 할당합니다.
;; bounding box 의 경계에 할당된 ID 는 무한히 확장하는 지역이기 때문에 계산에서 제거합니다.
;; 남은 ID 들의 넓이를 확인하고 그 중 가장 큰 넓이를 반환합니다.
...
;; 정렬을 따로 빼도 되지 않았을까?
;; 함수의 몸체가 map 하나일 경우 가독성이 떨어질수 있다고 생각되어서, map 안에 있는 함수를 빼는게 더 좋지 않을까?
;; 익명함수가 길어지면 보통 안좋음
...
;; Part 1 과 동일하게 bounding box 및 grid를 생성합니다.
;; grid 의 각 점을 순회하며, 해당 지점에서 모든 타겟 좌표의 거리의 총 합을 계산합니다.
;; 계산된 거리의 총 합이 threshold 미만인 지점들의 개수를 셉니다.

;; 다 계산하고 필터하는 방식보다 애초에 필터했다면?
;; 팁 : 추상화 하여 함수를 주입받는 형태를 만들어보기
